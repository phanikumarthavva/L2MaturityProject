    ...
    stage('Smoke Test') {
      steps {
        sh '''
          ...
          echo "Testing internal API service..."
          ...
          # DNS may take a few minutes, so retry
          for i in $(seq 1 20); do
            if curl -I --connect-timeout 10 "http://$WEB_LB"; then
              echo "Web is reachable: http://$WEB_LB"
              exit 0
            fi
          ...
          echo "Web LoadBalancer not reachable yet. Check AWS ELB target health."
          kubectl -n "$NAMESPACE" get svc prm-web
          kubectl -n "$NAMESPACE" describe svc prm-web
          exit 1
        '''
